...
public class PdmsTextParserTests
{
    ...
    public void AttributesStringDeduplication()
    {
        ...
        {
            var allocatedStringObjectCount =
                memory.GetTrafficFrom(memoryCheckPointA).Where(x => x.Type.Is<string>()).AllocatedMemory.ObjectsCount
                - stringAllocationsByLineParser;
        ...
        {
            var allocatedStringObjectCount =
                memory.GetTrafficFrom(memoryCheckPointB).Where(x => x.Type.Is<string>()).AllocatedMemory.ObjectsCount
                - stringAllocationsByLineParser;
